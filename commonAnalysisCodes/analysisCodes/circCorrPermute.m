...
% alpha1 and alpha2 are the two vectors of circular values. 
    ...
    for i = 1:nPerm
        alpha2P = alpha2(randperm(alphaLength));
        [corrCCPerm(i),~] = circ_corrcc(alpha1, alpha2P);
    end
    corrP = (length(find(abs(corrCCPerm) > abs(corrCC)))+1) / (nPerm+1);
if figureFlag==1
    histogram(corrCCPerm,'Normalization','count')
    hold on
    xline(abs(corrCC),'LineWidth',2,'Color','red')
    xlabel('Bins')
    ylabel('Counts')
