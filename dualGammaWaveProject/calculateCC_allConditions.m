...
for i = 1:numFreqLimits
    for j = 1:numTrials
        [waveVector(:,j,i),~,waveBounds{i,j}] = getWaveSegments(outputs{i,j},timeVals,5,3,[0.25 0.75], 25);
    end
end
...
allUniqueDirs(find(emptyCell==1)) = [];
alpha = cell2mat(allUniqueDirs);

% get monte Carlo CC
[corrCC,corrP] = circCorrPermute(alpha(1,:), alpha(2,:), 999,1);

%% 2. with 5 degree variation across the successive time points (to detect spiral waves)
% done for waves detected with gamma bursts and threshold fraction set at 50 for a 5 deg variance between each successive time point
% CC is done for mean wave angles
numFreqLimits = 2;
numTrials = size(outputs,2);
numTimePoints = length(timeVals);
...
% get overlapping waves
for j = 1:numTrials
    [newBounds{j},allDirSg(j,:),allDirFg(j,:),~,~] = getOverlappingWaves(waveVector(:,j,1),waveBounds{1,j},waveVector(:,j,2),waveBounds{2,j},0.5);
end
...
% get monte Carlo CC
[corrCC,corrP] = circCorrPermute(allDirSg, allDirFg, 999,1);
